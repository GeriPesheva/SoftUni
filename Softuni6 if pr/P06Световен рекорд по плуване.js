...
function worldRecord(input) {
    let recprdOfSeconds = Number(input[0]);
    let distanceMt = Number(input[1]);
    let timeSeconds = Number(input[2]);
    let timeIvan = distanceMt * timeSeconds;
    let timeZabav = Math.floor(distanceMt / 15) * 12.5;
    let timeSumIvan = timeIvan + timeZabav;
    let result = ( timeSumIvan-recprdOfSeconds );
  
    if ( timeSumIvan>recprdOfSeconds) {
        console.log(`No, he failed! He was ${result.toFixed(2)} seconds slower.`);
}  
    else {
      
        console.log(`Yes, he succeeded! The new world record is ${timeSumIvan.toFixed(2)} seconds.`);
}
