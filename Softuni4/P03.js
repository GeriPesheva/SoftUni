...
function calcolatorDeposit(input) {
    let depositSum=Number(input[0]);
    let depositTerm=Number(input[1]);
    let prosent = Number(input[2]);
    let prosentOf = prosent / 100;
    let sum = (depositSum * prosentOf) / 12;
    let totalSum = depositSum + (depositTerm * sum);

    console.log(totalSum);
}
calcolatorDeposit(["200 ", "3 ", "5.7 "]);